...
func (p *QueryParams) Serve(ctx *faygo.Context) (err error) {
	logger.Debug("request device type query controller")
	result := controller.NewResult()
	defer ctx.JSON(200, result, true)

	cli, exist := controller.ServiceMap.Get("core_cli")
	if !exist {
		logger.Info("Trying to list device type failed, grpc was not connected")
		return
	}
	...
	req := &protos.GetDeviceTypesRequest{}
	deviceTypeList, err := cli.(protos.CoreServiceClient).GetDeviceTypeList(_ctx, req)
	if err != nil {
		result.Code = -1
		result.Msg = err.Error()
		logger.Errorf("get device type list by core service failed: %s", err.Error())
		return
	}
	result.Data = deviceTypeList
	return
}

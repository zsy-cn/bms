...
func (p *DeleteParams) Serve(ctx *faygo.Context) (err error) {
	logger.Debug("request device type delete controller")
	...
	if !exist {
		logger.Info("Trying to delete device type failed, grpc was not connected")
		return
	}

	_ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
	_, err = cli.(protos.CoreServiceClient).DeleteDeviceType(_ctx, req)

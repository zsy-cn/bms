...
import (
	...
	"github.com/zsy-cn/bms/service/parser/env_yingfeichi_01"
...
func init() {
	...
	viper.SetDefault("bind-addr", conf.ParserEnvYingfeichi01BindAddr)
	// 注意service地址要能被consul访问到, 且端口与服务监听端口保持一致
	viper.SetDefault("service-addr", conf.ParserEnvYingfeichi01ServiceAddr)
	viper.SetDefault("service-port", conf.ParserEnvYingfeichi01ServicePort)
	viper.SetDefault("service-name", "environ_monitor_sensingtree104")
}
...
func main() {
	...
	parserService, err = env_yingfeichi_01.New(logger)
	if err != nil {

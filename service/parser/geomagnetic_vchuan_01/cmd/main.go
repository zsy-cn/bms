...
import (
	...
	"github.com/zsy-cn/bms/service/parser/geomagnetic_vchuan_01"
...
func init() {
	...
	viper.SetDefault("bind-addr", conf.ParserGeomagneticVchuan01BindAddr)
	// 注意service地址要能被consul访问到, 且端口与服务监听端口保持一致
	viper.SetDefault("service-addr", conf.ParserGeomagneticVchuan01ServiceAddr)
	viper.SetDefault("service-port", conf.ParserGeomagneticVchuan01ServicePort)
	viper.SetDefault("service-name", "geomagnetic_VSP-200")
}
...
func main() {
	...
	parserService, err = geomagnetic_vchuan_01.New(logger)
	if err != nil {

...
import (
	...
	"github.com/zsy-cn/bms/service/parser/manhole_cover_gti_01"
...
func init() {
	...
	viper.SetDefault("bind-addr", conf.ParserManholeCoverGti01BindAddr)
	// 注意service地址要能被consul访问到, 且端口与服务监听端口保持一致
	viper.SetDefault("service-addr", conf.ParserManholeCoverGti01ServiceAddr)
	viper.SetDefault("service-port", conf.ParserManholeCoverGti01ServicePort)
	viper.SetDefault("service-name", "manhole_cover_18481500000")
}
...
func main() {
	...
	parserService, err = manhole_cover_gti_01.New(logger)
	if err != nil {

...
import (
	...
	"github.com/zsy-cn/bms/service/parser/temperature_weichuan_01"
...
func init() {
	...
	viper.SetDefault("bind-addr", conf.ParserTemperatureWeichuan01BindAddr)
	// 注意service地址要能被consul访问到, 且端口与服务监听端口保持一致
	viper.SetDefault("service-addr", conf.ParserTemperatureWeichuan01ServiceAddr)
	viper.SetDefault("service-port", conf.ParserTemperatureWeichuan01ServicePort)
	viper.SetDefault("service-name", "temperature_AN-101C")
}
...
func main() {
	...
	parserService, err = temperature_weichuan_01.New(logger)
	if err != nil {

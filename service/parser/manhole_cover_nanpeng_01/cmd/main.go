...
import (
	...
	"github.com/zsy-cn/bms/service/parser/manhole_cover_nanpeng_01"
...
func init() {
	...
	viper.SetDefault("bind-addr", conf.ParserManholeCoverNanpeng01BindAddr)
	// 注意service地址要能被consul访问到, 且端口与服务监听端口保持一致
	viper.SetDefault("service-addr", conf.ParserManholeCoverNanpeng01ServiceAddr)
	viper.SetDefault("service-port", conf.ParserManholeCoverNanpeng01ServicePort)
	viper.SetDefault("service-name", "manhole_cover_NP-SMC06")
}
...
func main() {
	...
	parserService, err = manhole_cover_nanpeng_01.New(logger)
	if err != nil {

...
import (
	...
	"github.com/zsy-cn/bms/service/parser/geomagnetic_weichuan_01"
...
func init() {
	...
	viper.SetDefault("bind-addr", conf.ParserGeomagneticWeichuan01BindAddr)
	// 注意service地址要能被consul访问到, 且端口与服务监听端口保持一致
	viper.SetDefault("service-addr", conf.ParserGeomagneticWeichuan01ServiceAddr)
	viper.SetDefault("service-port", conf.ParserGeomagneticWeichuan01ServicePort)
	viper.SetDefault("service-name", "geomagnetic_AN101C")
}
...
func main() {
	...
	parserService, err = geomagnetic_weichuan_01.New(logger)
	if err != nil {
